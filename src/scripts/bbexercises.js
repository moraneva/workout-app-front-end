...
for (var i = 0; i < 1092; i = i + 15) {
    promiseArray.push(new Promise((resolve, reject) => {
        ...
        fetch('http://www.bodybuilding.com/exercises/ajax/getfinderdata/', {
                    ...
                    function (err, window) {
                        ...
                        window.$('.exerciseName p span a').each(function () {
                            if (window.$(this).parent().parent().text().startsWith("Muscle"))
                                musclesTargeted.push(this.text.substring(1, this.text.length - 1));
                            else if (window.$(this).parent().parent().text().startsWith("Equipment"))
                                equipmentTypes.push(this.text.substring(1, this.text.length - 1));
                        });
